...
module.server = httpserver.new():setPort(serverPort):setCallback(function(method, path, headers, body)
    ...
    elseif attr and attr.mode == "directory" then
        ...
        for k in fs.dir(file) do
            ...
            if k:sub(1,1) ~= "." then
                if fattr then
                    data = data .. string.format("    %-12s %s %7.2fK <a href=\"http://" .. headers.Host .. path .. "/%s\">%s</a>\n", fattr.mode, fattr.permissions, fattr.size / 1024, k, k)
                else
                    data = data .. "    ?? " .. k .. " ??\n"
                end
            end
